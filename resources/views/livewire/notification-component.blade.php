<div class="relative" x-data="{ open: false }">
    ...
    <div x-show="open" class="absolute right-0 mt-2 bg-white text-black rounded shadow w-64 z-50">
        <div class="p-2 text-sm font-semibold text-gray-700 border-b">Unread Messages</div>
        <ul>
            @forelse($unreadMessages as $message)
                <li class="p-2 hover:bg-gray-100 cursor-pointer"
                    wire:click="markAsRead({{ $message->sender_id }})"
                    @click="open = false">
                    <span>From: {{ optional($message->sender)->name ?? 'Unknown' }}</span>
                </li>

            @empty
                <li class="p-2 text-gray-500">No new messages</li>
            @endforelse
        </ul>
    </div>
</div>
