<x-app-layout>
    ...
    {{-- DAILY DASHBOARD --}}
        <section class="max-w-7xl mx-auto text-center py-20">
            <h2 class="text-4xl font-extrabold text-purple-300 mb-12">"{{ $quote }}"</h2>
                <p class="text-lg italic text-gray-300 mb-8">📅 Today’s Dashboard</p>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-10">
                    {{-- Workout --}}
                        <div class="bg-white/10 rounded-2xl p-6 shadow-lg backdrop-blur-md">
                            <h3 class="text-2xl font-bold text-purple-200 mb-4">💪 Today’s Workout</h3>
                                @if($todayWorkout)
                                <p class="text-gray-300 font-semibold">{{ $todayWorkout->title }}</p>
                                <p class="text-gray-300 mb-4">{{ $todayWorkout->description }}</p>
                                    <form action="{{ route('student.workout.complete') }}" method="POST">
                                        @csrf
                                        <input type="hidden" name="workout_plan_id" value="{{ $todayWorkout->workout_plan_id }}">
                                        <button type="submit" class="bg-purple-600 text-white px-4 py-2 rounded-full hover:bg-purple-700">
                                            Mark as Done ✅
                                        </button>
                                    </form>
                                @else
                                    <p class="text-gray-400">No workout scheduled today.</p>
                                @endif
                            </div>
                            {{-- Progress --}}
                            <div class="bg-white/10 rounded-2xl p-6 shadow-lg backdrop-blur-md">
                                <h3 class="text-2xl font-bold text-purple-200 mb-4">📊 Progress</h3>
                                <p class="text-gray-300">Completed: <span class="font-bold">{{ $completedDays }}</span> / {{ $totalDays }}</p>
                                <p class="text-gray-300">Rate: <span class="font-bold">{{ $completionRate }}%</span></p>

                                <div class="mt-4 w-full bg-gray-700 rounded-full h-4">
                                    <div class="bg-purple-500 h-4 rounded-full" style="width: {{ $completionRate }}%"></div>
                                </div>
                            </div>
                    ...
                    <h2 class="text-4xl font-extrabold text-purple-300 text-center mb-12">💪 Recommended Exercises</h2>

                    @if($recommendedExercises->isEmpty())
                        <p class="text-gray-300 text-center">No exercises available at the moment.</p>
                    @else
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                            @foreach($recommendedExercises as $exercise)
                                <div class="bg-white/10 rounded-xl p-4 shadow-lg backdrop-blur-md hover:bg-white/20 transition transform hover:-translate-y-1">
                                    <h4 class="text-lg font-semibold text-purple-200 mb-2">{{ $exercise->title }}</h4>

                                    @if($exercise->video)
                                        <video controls class="w-full h-32 object-cover rounded mb-2">
                                            <source src="{{ asset('storage/'.$exercise->video) }}" type="video/mp4">
                                            Your browser does not support the video tag.
                                        </video>
                                    @endif

                                    <a href="{{ route('exercises.show', $exercise->id) }}" class="mt-2 inline-block bg-purple-600 text-white px-3 py-1 rounded-full hover:bg-purple-700 transition text-sm">
                                        View Details →
                                    </a>
                                </div>
                            @endforeach
    ...
    <section class="grid md:grid-cols-2 gap-12 items-center max-w-7xl mx-auto py-20">
        <div class="space-y-6">
    ...
    <section class="text-center max-w-7xl mx-auto pt-20 relative">
        ...
        <div x-data="{
            ...
            <button @click="prev" class="absolute top-1/2 left-0 -translate-y-1/2 bg-purple-600/50 hover:bg-purple-700 text-white p-3 rounded-full">‹</button>
            <button @click="next" class="absolute top-1/2 right-0 -translate-y-1/2 bg-purple-600/50 hover:bg-purple-700 text-white p-3 rounded-full">›</button>
        </div>
    </section>

    {{-- MAP SECTION --}}
    <section class="text-center max-w-7xl mx-auto py-20">
